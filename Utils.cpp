...
//Función que realiza la espera de las nuevas conexiones y las va atendiendo de manera asincrona
...
/*Función que atiende las peticiones de las conexiones de manera asincrona realiza una copia del cliente con la ID recibida
luego trabaja con los datos directamente sobre la copia y atiende la petición de recibir el mensaje, cuando ha atendido la
petición borra el mensaje de la lista*/
...
//Devuelve la cantidad de mensajes que hay en la lista
...
//Comprueba que existan mensajes en la lista
bool checkMSG(int clientID) {

	return (getCountMSG(clientID) > 0);
}

/*Quita el mensaje de la lista de la ID recibida en caso de que siga habiendo mensajes en la lista, una vez realizado el
pop se libera la memoria del mensaje*/
...
//Pone la variable shutdown en true para cerrar el programa
void shutOff() {
